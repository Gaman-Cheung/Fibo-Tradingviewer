...
// ================= Supabase 配置区域 =================
        ...
        function addV6Row(n='', h='', l='', c='', instrumentId='', entry='', previous='', baseline='current', previousMode='', previousDate='') {
            ...
            tr.innerHTML = `
                ...
                <td class="col-previous"><div class="previous-shell"><input type="number" class="previous" value="${previous}" placeholder="Prev" data-fibo-input="handlePreviousCloseInput(this)"><button type="button" class="previous-mode-button" data-fibo-click="togglePreviousCloseMode(this)"><span class="material-icons">cloud_sync</span></button></div></td>
                <td class="col-baseline"><select class="baseline" aria-label="Percentage Baseline" data-fibo-change="calcV6(this)"><option value="current" ${baseline==='current'?'selected':''}>Current</option><option value="entry" ${baseline==='entry'?'selected':''}>Entry</option><option value="previous" ${baseline==='previous'?'selected':''}>Prev</option></select></td>
        ...
        const HELP_TOPICS = {
            ...
            macd: { title:'MACD Trend · 动量状态', html:`<h3>12/26/9 指标怎么读</h3><p><strong>DIF</strong> 是快线，<strong>DEA</strong> 是 DIF 的平滑信号线，当前系统的 <strong>Histogram = 2 × (DIF − DEA)</strong>。DIF 向上穿越 DEA 是 Golden Cross（金叉），向下穿越是 Death Cross（死叉）；零轴反映中期动量位于正区还是负区。</p><p>不同券商软件的红柱、绿柱配色可能相反，不要只按颜色判断。应读取柱体的正负、方向以及正在扩张还是缩短，并结合 DIF、DEA 的位置与斜率。</p><h3>四种人工选项与分值</h3><p><strong>Bullish Divergence +2</strong>（兼容存储值 <code>divergence</code>）；<strong>Bullish +1</strong>；<strong>Wait/Flat 0</strong>（兼容存储值 <code>neutral</code>）；<strong>Bearish -1</strong>。MACD 与 RSI 合并为 Momentum，合计仍封顶 <strong>+3</strong>。</p><h3>人工确认 · Bullish / Bearish</h3><p><strong>Bullish</strong> 建议至少两项一致：DIF 位于 DEA 上方、DIF 与 DEA 双线上行、正柱持续扩张、金叉后持续确认。价格突破短期压力只能作为人工辅助确认，不能单独决定 MACD 选项。</p><p><strong>Bearish</strong> 建议至少两项一致：DIF 位于 DEA 下方、DIF 与 DEA 双线下行、负柱持续扩张、死叉后持续确认。价格跌破短期支撑只能作为人工辅助确认。</p><p>刚交叉但双线仍粘合、柱体很短时，无论金叉或死叉都先选 <strong>Wait/Flat</strong>。</p><h3>什么时候选 Wait/Flat</h3><p>双线缠绕或走平；反复交叉；柱体接近零轴；刚交叉且只有一根、尚未确认；负柱缩短但 DIF 仍低于 DEA；正柱缩短但 DIF 仍高于 DEA；疑似但未确认的背离；数据不足或无法判断。动能改善不等于方向已经确认。</p><h3>简明决策流程</h3><div class="formula"><code>先确认底背离？</code><br>是 → 人工确认后选 Bullish Divergence<br>否 → Bullish 至少两项一致 / Bearish 至少两项一致<br>其余 → Wait/Flat</div><h3>自动建议与人工判断</h3><p>Fresh Golden Cross 继续直接提示 Bullish，Fresh Death Cross 继续直接提示 Bearish。没有新交叉时，DIF &gt; DEA 且位于零轴上方仍为 Bullish，DIF &lt; DEA 且位于零轴下方仍为 Bearish。</p><p>在<strong>零轴下方</strong>，只有 DIF &gt; DEA、DIF 与 DEA <strong>双线上行</strong>且<strong>正柱继续扩张</strong>时，才确认 Bullish 动能延续；在<strong>零轴上方</strong>采用镜像规则，双线下行且<strong>负柱继续扩张</strong>才确认 Bearish 动能延续。柱体缩短、双线不同向或上一点不足时仍提示 Wait/Flat。</p><p>Official Close 与前一个正式交易日比较；Current Preview 与最新正式收盘比较。Current 涨幅本身不会直接决定 MACD 标签。建议仍只是候选，只有点击 <strong>Apply Suggestion</strong> 才写入下拉框；更严格的人工复核后，用户可以保留 Wait/Flat。</p><p><strong>Official Close</strong> 使用最新正式收盘，是客观确认口径；<strong>Current Preview</strong> 会临时追加 Current，只是条件预演。切换口径不会自动修改选项或评分。</p><h3>背离边界</h3><p><strong>Bullish Divergence</strong> 只用于人工确认的底背离：价格形成更低低点，同时 DIF 形成更高低点。系统扫描最近60个正式交易日，以左右各两个交易日的五点拐点提供候选；Current Preview 不参与，候选也不会自动写入。</p><p>顶背离是价格创新高而 DIF 形成更低高点，绝不能选择加2分的 Bullish Divergence；弱势已经确认时选 Bearish，否则选 Wait/Flat。最终仍应查看完整 K 线人工确认。</p>` },
