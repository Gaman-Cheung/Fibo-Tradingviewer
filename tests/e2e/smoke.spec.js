...
window.supabase={createClient(){
 const chain={select(){return this},eq(){return this},single(){return Promise.resolve({data:null,error:{code:'PGRST116'}})},upsert(){return Promise.resolve({data:null,error:null})}};
...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(() => {
    localStorage.setItem('tv_instrument_pool_v1', JSON.stringify({ version:1, items:[
      { id:'e2e-a', ticker:'E2E', order:0, status:'active' }
    ], tombstones:[] }));
...
test('terminal controller switches tabs and persists shared Pro Tips', async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name === 'iphone') await page.locator('#mobileNavTips').click();
