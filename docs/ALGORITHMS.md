...
- The chart names the unchanged 60/40/20 boundaries `Hot`, `Wind` and `Cold`. Their green/yellow/red dashed presentation is display-only: Hot/Cold describe breadth temperature, while Wind marks a two-way transition watch line. They are not another score, probability, confirmed overbought/oversold or reversal rule, direction forecast or trading trigger.
