...
- The Pulse chart uses a Google-green dashed `Hot` line at 60, a Google-yellow dashed `Wind` line at 40 and a Google-red dashed `Cold` line at 20; the 80 guide remains neutral gray. A three-column footer places the first date, the three centered line keys and latest date on one line without increasing the shared Market Context frame height. Mobile abbreviates the labels to `H60`, `W40` and `C20`.
