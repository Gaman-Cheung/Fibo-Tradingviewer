...
- FIBO Market Pulse: pure `scripts/market_pulse.py` owns the four 25% breadth groups and member flags; `src/core/market-pulse-repository.js` reads only precomputed snapshots and 50-row member pages; `src/pulse/` owns browser normalization, chart geometry and help; `src/apps/market-pulse-controller.js` owns the isolated four-card dashboard, Canvas and detail filters. The Market Context router defaults to Pulse but keeps all three Radar scopes unchanged.
