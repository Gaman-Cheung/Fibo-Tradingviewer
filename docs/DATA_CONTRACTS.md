...
- `market_sync_checkpoint` has one `(provider,scope)` row for idempotent backfill progress and global freshness. A cursor advances only after every batch for that date succeeds, except for the explicitly recorded CN_ETF provider-prefix case below. After `daily / all`, a read-only audit requires CN_A/CN_INDEX/CN_PULSE/CN_ETF and the Index/Pulse/two ETF snapshots to be `ok` on the A-share run's latest official date; any mismatch fails the command without rewriting checkpoints, rolling back rows or deleting the prior valid snapshot.
...
- The Market Context browser defaults to `MARKET_PULSE`; all three Radar scopes remain lazy and cache only for the page session. Cache entries expire after five minutes or at a Shanghai calendar-day boundary; returning the page to the foreground checks only the active Scope, while an expired inactive Scope refreshes when selected. Automatic refresh failure retains that Scope's last valid cards and official date. No selection is persisted to localStorage or cloud state, and one Scope failure cannot clear another Scope or block Look First.
