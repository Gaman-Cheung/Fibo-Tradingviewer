...
- New additive keys are `(user_id,instrument_id)` for bindings, `(provider,market,code,trade_date)` for closes, `(provider,market,code)` for sync state, and `user_id` for Tracker state.
- Official front-adjusted close is the only historical price stored. Current and five-day VR remain manual per permanent ID.
- Service-role credentials may exist only in GitHub Secrets/server-side jobs.
